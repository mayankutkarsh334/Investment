...
1. **Is the fee justified?** On the honest realized record, **no** — the premium over the 0.20% index fund buys *negative* alpha on the 6.8-year matched window (M1's −0.22%/yr), and the study's highest turnover adds a hidden ~0.2%/yr drag. The fee case rests on Cheenu Gupta's 3.6-year alpha persisting — a manager Module 5 has since confirmed is *verifiably competent* (a genuine 2020 COVID crisis win, replicated across four HSBC funds), so a **credible bet rather than a coin-flip** — but a bet nonetheless, against a negative realized base case. This is a softer version of the Franklin pattern in a cheaper wrapper.
...
**HSBC is the first fund since Franklin US whose fee premium buys negative alpha on the honest full-window record.** On the matched 6.8-year Nifty Midcap 150 index-fund comparison, the fund *lost* to the buyable passive (−0.22%/yr), so paying 0.36–0.55% for it made you worse off. The only way the fee justifies itself is if Cheenu Gupta's 3.6-year +4.99% persists — and here Module 5 upgrades the warranty: she is **not an unknown** but a verifiably competent manager (a genuine 2020 COVID crisis win — +18.3 alpha in small-cap, +13.1 in multicap, cushioning both crashes — replicated in style across four HSBC funds). So the forward case is a *credible bet*, not the coin-flip an untested streak would be — but it remains a bet, because the *realized* base case is negative and the record is lumpy (2 negative-alpha years in 4).
...
- **Negative fee-for-alpha base case** — the premium buys negative value on the honest 6.8-yr window (−₹2.4L vs index); first since Franklin; forward case rests on Cheenu's 3.6-yr streak persisting (M5: a verifiably competent manager, so a credible bet — but the realized base case is still negative)
...
| **ER vs verified alpha (index-fund test)** | **Critical** | **2.75** *(nudged from 2.5 post-M5)* | Matched-window alpha −0.22% + ~0.2% turnover drag → true premium ~0.55% buys *negative* value on the honest record (−₹2.4L vs index); the forward case rests on Cheenu's lumpy 3.6-yr +4.99 — **nudged up from 2.5 because M5 verified her competence** (2020 crisis win, replicated ×4 funds), raising the warranty above a coin-flip; held below 3.0 because the *realized* base case is still negative. First negative base case since Franklin |
...
**Module 4 Score: ~3.6 / 5** — a genuine split personality: the **best capacity position of the three midcaps** (sweet-spot size, flat flows, unlimited runway, zero deployment strain) colliding with the **worst fee-for-alpha case** (a negative base case on the honest window, worsened by the study's highest turnover). The two roughly offset, landing HSBC at **the same ~3.6 as Nippon — for mirror-opposite reasons**: Nippon has a proven fee on a dying capacity trajectory; HSBC has a credible-but-unwon-bet fee on a pristine capacity trajectory. **Module 5 has since resolved the key conditionality — Cheenu Gupta is verifiably competent (a real 2020 crisis win, replicated ×4 funds), which nudged the fee-for-alpha row 2.5→2.75; the module overall remains ~3.6, as the realized base case is still negative. Module 6 confirmed the "market isn't buying" franchise signal and carried the HSBC AMC verdict forward (~3.4).**
