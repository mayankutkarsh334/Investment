...
> **Provisional Module 1 score: ~3.4 / 5** *(revised twice — see the correction notes below)* (weight 20% in the multi-asset re-weight). **Scores in this study are NOT directly comparable to the four equity categories** — the module measures a risk-reduction product against a *blended* benchmark and a DIY basket, not a single equity index against peers.

> ### ⚠ CORRECTION APPLIED AFTER MODULE 2 (Edelweiss cross-module discipline)
> **This module originally scored the allocation engine 2.0 and concluded the "Internal Proprietary Model … has not earned its keep in any year of its life." That framing was one-sided: it measured the allocation effect on the *return* axis only.** M2 measured the same tilt on the risk axis and found it materially valuable — the fund-weight passive replica ran **5.51% volatility and −7.21% max drawdown against the benchmark's 7.45% and −10.88%**, and in the Jan-2026 tri-asset crash the fund's metals *underweight* was worth **+5.73pp of drawdown protection versus its own benchmark**. Risk-adjusted, the allocation effect is roughly **neutral on Sharpe (1.14 → 1.08)** and **clearly positive on Calmar (1.38 → 1.72)**.
>
> **The core finding stands** — the dial does not move, and selection rather than allocation is where the value is. But the correct characterisation is **"a permanent defensive tilt, not a dynamic engine — it gives up return and buys back an almost exactly offsetting amount of risk,"** not "the model destroys value." **The Allocation-timing sub-score is corrected 2.0 → 2.5.** All other figures in this module are unchanged and were verified against M2's independent computation.

> ### ⚠ SECOND CORRECTION APPLIED AFTER MODULE 4 — the headline claim was overstated
> **This module called WOC "the study's first unambiguous post-tax DIY win." That framing is withdrawn.** M4 re-ran the identical post-tax calculation for all six studied funds over WOC's *exact* window and found **every one of them beats the DIY 65/25/10 basket — and WOC's margin is the smallest of the six**: +₹66,184 on ₹10L, versus SBI +₹91,068, ABSL +₹1,06,573, UTI +₹1,24,119, Nippon +₹2,22,287 and Quant +₹3,69,517.
>
> **The arithmetic in this module was correct** — the +1.55pp post-tax edge at ~43% less volatility stands, and the risk qualifier was stated. What was wrong was the word *"unambiguous"* and the implied uniqueness, which came from comparing against earlier modules' figures computed over *their own* windows with a Nifty-50 equity leg. **The claim survives only in risk-adjusted form — where it is genuinely the strongest in the study: WOC's post-tax Sharpe advantage over DIY is +0.81, versus Quant's +0.63 and ABSL's +0.22.** **The "Beat the DIY static basket" sub-score is corrected 4.5 → 4.0.**
>
> **Net effect of both corrections: Module 1 Overall returns to ~3.4** — the M2 correction lifted it +0.075, the M4 correction lowers it −0.075. Two offsetting corrections in opposite directions, landing back at the original figure.

> **The one-line context:** WOC is the study's cleanest **risk-adjusted** record and holds its **largest risk-adjusted post-tax DIY win** *(M4 correction: its absolute DIY margin is the smallest of the six)* — 16.90% since-inception CAGR at **5.65% volatility** and a **−6.08% max drawdown** (Sharpe 1.84, the best of the six funds studied), beating a DIY 65/25/10 basket by **+1.55pp post-tax** at roughly **40% less volatility**. It also has the **most honest benchmark in the study** — a genuine five-leg AMFI Tier-I composite pulled from the SID, not a lazy equity index. And then the decomposition arrives: the **+1.92%/yr alpha vs that benchmark splits into SELECTION +4.47%/yr and ALLOCATION −2.56%/yr**, and the allocation effect is **negative in every single one of the fund's four calendar years**. For a fund whose marketing headline is an "Internal Proprietary Model to provide direction" on the relative attractiveness of asset classes, the model has *subtracted return* every year it has run, while the security and instrument selection has added value every year. *(M2 correction: on the risk axis that same tilt is roughly Sharpe-neutral and Calmar-positive — read it as a permanent defensive posture rather than a value-destroying one.)* Returns-based style analysis confirms why: effective equity beta has sat in a **27.6%–29.5% band for the fund's entire life** despite an SID mandate permitting **10%–80%**. The fund is excellent. The allocation engine — the category's whole skill claim — is, on this evidence, not the reason.
...
2. **✅ The largest risk-adjusted DIY win in the study (+0.81 Sharpe points over the basket): +1.55pp/yr over 65/25/10 at 43% less volatility** — and +2.31pp over the AMC's own "efficient portfolio" mix. *(M4 correction: the absolute margin is the smallest of the six funds; the risk-adjusted margin is the largest.)*
...
### ❌ Against
1. **⚠ THE FINDING: the allocation engine subtracts *return*, and has done so every single year.** Alpha decomposes into **selection +4.47%/yr and allocation −2.56%/yr**; the allocation effect is negative in 2023, 2024, 2025 and 2026 without exception. The fund's advertised "Internal Proprietary Model" for asset-class attractiveness has not added a rupee of return in any year of its life. *(Corrected after M2: on the risk axis the same tilt is roughly Sharpe-neutral and Calmar-positive — it is a permanent defensive posture, not a value-destroying one. It is still not the dynamic allocation engine being sold.)*
...
xychart-beta
    ...
    y-axis "Score" 0 --> 5
    bar [3.5, 4.0, 4.0, 2.5, 3.0, 2.0, 3.5, 3.5, 3.5, 4.5]
```
...
| **Beat the DIY static basket, post-tax** *(Critical)* | 15% | **4.0** *(rev. from 4.5 by M4)* | ✅ Beats every DIY basket post-tax (+1.55pp over 65/25/10) at ~43% less volatility, and has the **largest risk-adjusted DIY edge in the study (+0.81 Sharpe points)** — ⚠ but **M4 showed all six studied funds beat DIY over this window and WOC's absolute margin is the smallest of the six** (+₹66,184 vs Quant's +₹369,517). Also held down by the 3.19y window, the unresolved ER, and a 10Y forward projection that is a coin-flip |
| **Allocation-timing contribution** *(High — the category's skill claim)* | 15% | **2.5** *(rev. from 2.0)* | ❌ **The module's decisive negative, softened by M2.** Allocation effect **−2.56%/yr on return, negative in 4 years of 4**; effective equity beta parked in a 1.9pp band inside a 70pp mandate — no timing calls are visible at all. **Revised up 0.5 because M2 showed the same tilt is roughly Sharpe-neutral and Calmar-positive** (it bought −1.9pp of volatility and +3.7pp of drawdown protection for the return it gave up). A static defensive tilt, not a dynamic engine — but not value-destroying either |
...
| **Module 1 Overall** | **100%** | **~3.4 / 5** *(revised twice — see the correction boxes)* | **The best risk-adjusted record in the study and a genuine, if narrow, post-tax DIY win — achieved despite, not because of, its asset-allocation engine.** Selection carries the fund; allocation has cost it return every year while buying back a near-equal amount of risk; and the two decisive stress tests are missing, partly offset by a tri-asset crash it handled better than any peer. Not comparable to equity-category Module 1 scores |
...
| **WOC Multi Asset** | **~3.4 / 5** | **Best Sharpe and best drawdowns of the six; the smallest absolute DIY win but the largest risk-adjusted one — with a static defensive allocation tilt that costs return and buys back risk** |
...
Three things must be resolved before this record can be relied on. **The 24-month tax gate** — this fund is not equity-taxed, and a redemption inside two years is taxed at slab, which erases the entire DIY advantage. **The ER** — M4 found five sources reporting 0.39% to 0.67%, and that spread alone is the difference between winning and losing the 10-year DIY test on projected mechanics. **And the missing bear market** — 3.19 years, no 2020, no 2022, and a conservative book that has never had to prove it can *recover*, only that it can *avoid*. Module 1's verdict: **the outcomes are the best in the study; the machinery producing them is not the machinery advertised; and the sample is too short to know which of the two matters more.**

## One-Line Verdict

**The study's best risk-adjusted record and its largest risk-adjusted post-tax DIY edge — though its smallest in absolute rupees — 16.90% CAGR at 5.65% volatility, a −6.08% worst drawdown fully recovered, the shallowest cushioning of both stress windows of any fund studied, and the only honest benchmark in the category — all of it delivered by security selection (+4.47%/yr) *in spite of* an asset-allocation engine that has subtracted return (−2.56%/yr) in every single year of the fund's life, inside a mandate whose 70-point equity range it has used less than two points of.**

---

*Module 1 complete. Provisional score **3.4/5** — revised twice and back to its original figure: **+0.075 after M2** (allocation-timing 2.0 → 2.5) and **−0.075 after M4** (Beat-DIY 4.5 → 4.0). See the two correction boxes at the top of this file. **Method:** self-computed from MFAPI scheme **151745** (774 NAVs, 22-May-2023 → 31-Jul-2026). Blended benchmark taken **verbatim from the SID** (AMFI portal doc 13647: BSE 500 TRI 30% + CRISIL Short Term Bond 50% + gold 16% + silver 1% + iCOMDEX Composite 3%) and built from **Motilal Oswal Nifty 500 Index Fund (147625)** as the BSE-500 proxy, **HDFC Short Term Debt (119016)** as the CRISIL-ST-Bond proxy, **SBI Gold (119788)**, and **Nippon India Silver ETF FoF (149760)**; the 3% iCOMDEX leg is proxied by gold and flagged as an approximation. Allocation-vs-selection split via constrained returns-based style analysis (R² 0.844) plus a Brinson-style replica; both daily- and annually-rebalanced conventions reported. Post-tax model uses full cost-basis tracking through each DIY rebalance at a 30% slab. Like-for-like window re-computed for SBI (119843), Nippon (148457), Quant (120821), UTI (120760), ABSL (151307).*
