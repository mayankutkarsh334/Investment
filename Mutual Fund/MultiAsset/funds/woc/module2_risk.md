...
| **Debt-sleeve duration + credit** | ⚠ **CORRECTED BY M3 — "structurally short by design" is only half true.** The *benchmark* leg is **CRISIL *Short Term* Bond (50%)**, but M3's holdings work found **GOI 6.28% 2032 and GOI 6.94% 2036 — 6–10 year paper** — so the book is **not uniformly short** and the 2022 rate-shock inference is weaker than stated here. **Credit quality is good** (sovereign + T-bills, plus quasi-sovereign AAA: PFC/IRFC/NABARD/SIDBI, with modest NBFC reach in Muthoot and Bajaj Housing); **no credit event visible in NAV**, and the debt leg's realised drawdown never exceeded −0.53%. **Modified duration still undisclosed**, and the sleeve has never met a rate shock | ⚠ **M3 read; duration still open** |
| **Equity-taxation continuity risk** | ⚠ **Inverted, and it is a real risk.** WOC is **not** equity-taxed — at 26.4% net equity it is nowhere near the 65% line, and both VR and Groww state middle-tier mechanics (12.5% after **24 months**, slab before). So the risk is not *losing* equity status; it is **sliding to slab-on-all-gains** if the book ever drifts above the >65%-debt-and-money-market "specified mutual fund" threshold. ⚠ **REDUCED BY M3:** the **SID's own taxation table explicitly classifies the scheme as "other than Equity Oriented Scheme and other than Specified Mutual Fund"** (12.5% LTCG without indexation; unlisted units → 24-month qualifying period), and disclosed debt & money market is **~32%**, not ~50% — a buffer of roughly **33pp** to the >65% trigger, not the ~15pp estimated here. **M1's consequence still stands if it ever happened** (post-tax 15.08% → 12.22%, losing the DIY test), but the probability is materially lower. Still a standing review trigger given the 10–80% debt mandate | ⚠ **confirmed middle-tier; risk smaller than first assessed** |
...
7. **⚠ Equity-taxation continuity risk, inverted:** the fund is **not** equity-taxed, and a drift above the >65%-debt "specified mutual fund" threshold would push it to slab — M1 showed post-tax return falls 15.08% → 12.22% and it then **loses to the DIY basket.** *(M3 correction: the SID classifies the scheme as non-specified and the real buffer is ~33pp, not the ~15pp first estimated — the risk is smaller than stated, though the 10–80% debt mandate keeps it live.)*
...
| **Equity-taxation continuity risk** *(NEW)* | 3% | **3.0** *(reasoning corrected by M3; score unchanged)* | ⚠ Inverted risk: not equity-taxed at all, so the exposure is a slide to **slab**, which M1 showed costs 2.86pp/yr and loses the DIY test. **M3 confirmed from the SID that the scheme is classified "other than Specified Mutual Fund" with a ~33pp buffer to the >65%-debt trigger — not the ~15pp assumed here** — so the probability is lower than first assessed, though the 10–80% debt mandate keeps it a live review trigger. *(Correcting to 3.5 would move the module total by +0.015 — immaterial, so the score is held at 3.0.)* |
