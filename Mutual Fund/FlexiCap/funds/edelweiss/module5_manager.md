...
```mermaid
xychart-beta
    title "Fund Manager Eras — Edelweiss Flexi Cap (Dec 2007 to May 2026)"
    x-axis ["2007–2016 (JPMorgan era)", "2016–2021 (Transition/Patwardhan)", "Oct 2021–Present (Trideep)"]
    y-axis "Approximate Era Length (Years)" 0 --> 10
    bar [9, 5, 4.5]
```
...
```mermaid
xychart-beta
    title "Manager Tenure on Current Fund — Shortlisted Peers (Years)"
    x-axis ["PP (Thakkar)", "AB SL (Mahesh P)", "Quant (Tandon)", "HSBC", "JM (Ramanathan)", "Edelweiss (Trideep)", "BOI", "Union", "HDFC (Gopal A)"]
    y-axis "Years on Fund" 0 --> 14
    bar [13.0, 15.0, 10.0, 6.0, 4.5, 4.5, 4.0, 4.0, 0.5]
...
```mermaid
timeline
    title Trideep Bhattacharya Career Arc
    2000s : State Street Global Advisors (London)
          : Portfolio Manager — Institutional global equities
    2000s–2010s : UBS Global Asset Management (London)
               : Portfolio Manager — Asian emerging markets
    Mid-2010s : Motilal Oswal Securities (India)
              : Head of Research — led sell-side equity research
    Jan 2017 : Joined Axis AMC
             : Senior Portfolio Manager — Alternate Equities
             : Built Brand Equity PMS to ₹1000+ Cr AUM
    Sep 2021 : Joined Edelweiss AMC as Co-CIO Equities
    Oct 2021 : Took over Edelweiss Flexi Cap Fund management
    2023–24  : Elevated to CIO – Fundamental Equities
    May 2026 : Managing 3 Edelweiss equity funds; ₹3,320 Cr AUM on Flexi Cap
```

### Academic Credentials — Tier-1 Foundation

```mermaid
pie title "Credential Tiers Among Shortlist Lead Managers"
    "IIT + Top MBA + CFA (Trideep)" : 1
    "Top MBA + CFA (Thakkar, Ramanathan, Agarwalla)" : 3
    "CFA only / other combo" : 2
    "No disclosed top-tier credential" : 3
```
...
```mermaid
xychart-beta
    title "Trideep Bhattacharya — Performance Across 3 Edelweiss Equity Funds (CAGR %)"
    x-axis ["Flexi Cap 1Y", "Flexi Cap 3Y", "Flexi Cap 5Y", "Large & Mid 1Y", "Large & Mid 3Y", "Large & Mid 5Y", "Mid Cap 1Y", "Mid Cap 3Y", "Mid Cap 5Y"]
    y-axis "Returns (%)" 0 --> 30
    bar [9.6, 17.0, 17.16, 7.46, 17.93, 15.54, 10.04, 26.30, 22.83]
```
...
```mermaid
xychart-beta
    title "Regulatory Severity Scale — Shortlisted Fund Managers"
    x-axis ["Rajeev Thakkar (PP)", "Satish Ramanathan (JM)", "Gopal Agrawal (HDFC)", "Trideep (Edelweiss)", "Sandeep Tandon (Quant)"]
    y-axis "Regulatory Severity (1=clean, 5=severe)" 0 --> 6
    bar [1.0, 1.0, 1.0, 2.5, 5.0]
```
...
mindmap
  root((FAIR Framework))
    Forensics
      ...
      Related-party dealing analysis
      Corporate governance assessment
      Management quality assessment
    Acceptable Price
      PE/PB discipline
      Avoids narrative-driven expensive stocks
      Buys fundamentally strong at reasonable valuations
    Investment Style Agnostic
      Neither pure value nor pure growth
      Opportunistic across styles
      Adapts to market cycles without rigid bias
    Robustness
      Business model durability
      Earnings sustainability across cycles
      Avoids story-driven momentum
```
...
```mermaid
xychart-beta
    title "Investment Philosophy Transparency — Shortlisted Peers"
    x-axis ["PP (Buffett/Graham)", "Edelweiss (FAIR)", "JM (GEEQ)", "HDFC (generic)", "Quant (VLRT/black-box)"]
    y-axis "Philosophy Transparency Score (1-5)" 0 --> 5
    bar [5.0, 4.0, 3.5, 2.5, 1.5]
```
...
```mermaid
timeline
    title Ashwani Agarwalla Career Timeline
    2006–2012 : CARE Ratings, Pioneer Intermediaries, Adventity, JM Financials AMC
              : Early career in credit analysis and equity research
    Jun 2012 : Joined Baroda Pioneer Mutual Fund
             : Senior Equity Analyst and Fund Manager
             : Focus sectors — BFSI, Automotive
    Aug 2021 : Joined JM Financial Services as Senior VP
    Jun 2022 : Joined Edelweiss AMC as Fund Manager
             : Co-managing Edelweiss Flexi Cap + 1 other scheme
```
...
```mermaid
pie title "Estimated Portfolio Decision Authority"
    "Trideep Bhattacharya (Lead, CIO)" : 75
    "Ashwani Agarwalla (Co-manager)" : 20
    "Raj Koradia (Assistant)" : 5
```
...
```mermaid
xychart-beta
    title "Key-Man Risk Level — Shortlisted Peers (Higher = More Risk)"
    x-axis ["HDFC (team)", "Edelweiss", "Quant", "JM", "PP (Thakkar = firm identity)"]
    y-axis "Key-Man Risk (1=low, 5=high)" 0 --> 6
    bar [2.0, 3.0, 3.5, 4.0, 4.5]
```
