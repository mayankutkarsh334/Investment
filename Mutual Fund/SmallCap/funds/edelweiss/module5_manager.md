...
```mermaid
timeline
    title The Bhatia-Patwardhan House Swap — BOI and Edelweiss Intersect
    2019 : Edelweiss SC launches under Harshad Patwardhan (ex-JPMorgan CIO)
    2020-2021 : Patwardhan builds the explosive years (+36.9%, +70.2%)
    Dec 2021 : Trideep Bhattacharya takes over Edelweiss SC (CO-CIO Equities)
    2022 : Patwardhan LEAVES Edelweiss to become CIO of Union MF
    Oct 2024 : Dhruv Bhatia LEAVES BOI (where he built BOI SC's record) and JOINS Edelweiss SC
    2024-2026 : Edelweiss team = Trideep + Bhatia + Koradia
```
...
```mermaid
timeline
    title Trideep Bhattacharya — ~21-Year Global-to-India Arc
    2003-2010 : Portfolio Manager, State Street Global Advisors and UBS Global Asset Management (LONDON)
              : Global equity investing
    2010s : Head of Research, Motilal Oswal Securities (India)
    2010s-2021 : Senior Portfolio Manager - Alternate Equities, Axis AMC
               : Built the Axis PMS business
    Sep 2021 : Joins Edelweiss AMC as CO-CIO Equities
    Dec 2021 : Takes over Edelweiss Small Cap Fund
    2024-2026 : Elevated to President and CIO, Edelweiss AMC ; runs ~9 schemes
```
...
```mermaid
mindmap
  root((Edelweiss SC — 3-Skill Team))
    Trideep CIO oversight
      IIT + SP Jain + CFA
      Global PM London State Street UBS
      Portfolio construction, macro, risk
      FlexiCap Rank 4 proof
    Bhatia small-cap specialist
      Built BOI Small Cap record 2018-24
      Hands-on bottom-up stock picking
      The genuine SC pedigree
    Koradia forensic analyst
      Chartered Accountant
      Ex-Morgan Stanley research
...
```mermaid
xychart-beta
    title "Edelweiss Small Cap — Track-Record Attribution by Manager Era"
    x-axis ["Patwardhan (2019-21)", "Trideep (2021-26)", "Bhatia+Koradia (2024-26)"]
    y-axis "Years as Lead/Co" 0 --> 5
    bar [2.8, 4.5, 1.6]
```
...
```mermaid
xychart-beta
    title "Manager/AMC SEBI Action Severity — Studied Funds (Higher = More Severe)"
    x-axis ["DSP", "HSBC", "Edelweiss", "BOI", "Invesco"]
    y-axis "Severity (1-5)" 0 --> 6
    bar [1.0, 1.5, 3.0, 2.5, 5.0]
```
...
```mermaid
xychart-beta
    title "Lead-Manager Scheme Load — Studied Small Cap Funds"
    x-axis ["DSP (Sambre)", "HSBC (Manghat)", "Edelweiss (Trideep)", "BOI (Singh)"]
    y-axis "Number of Schemes" 0 --> 25
    bar [3, 7, 9, 23]
```
...
```mermaid
xychart-beta
    title "Key Person Risk Dimensions — Edelweiss Small Cap (Current)"
    x-axis ["AUM Outflow Risk", "Philosophy Continuity", "Track-Record Attribution", "Team Bandwidth", "Succession Depth"]
    y-axis "Risk Level (1=Low, 5=High)" 0 --> 6
    bar [2.5, 2.5, 3.0, 2.0, 2.5]
```
...
xychart-beta
    ...
    x-axis ["PP-Thakkar", "DSP-Sambre", "BOI FC-Singh", "Union", "Edelweiss", "BOI SC", "HSBC", "Invesco"]
    y-axis "M5 Score (1-5)" 0 --> 6
    bar [5.0, 3.9, 3.6, 3.4, 3.4, 3.3, 3.3, 2.8]
    line [3.4, 3.4, 3.4, 3.4, 3.4, 3.4, 3.4, 3.4]
```
